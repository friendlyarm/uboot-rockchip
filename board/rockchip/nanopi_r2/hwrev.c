...
/*
 * Board revision list: <GPIO2_B4 | GPIO2_A3>
 *  0b10 - NanoPi-R2
 *
 * Extended by ADC_IN1
 */
...
#define GPIO2_BASE	0xff230000
	struct rockchip_gpio_regs *regs = (void *)GPIO2_BASE;

	/* B4, A3: input mode */
	clrbits_le32(&regs->swport_ddr, (1 << 12) | (1 << 3));
	val = readl(&regs->ext_port);
	pcb_rev  = ((val >>  3) & 1) << 4;
	pcb_rev |= ((val >> 12) & 1) << 5;
...
const char *get_board_name(void)
{
	...
	switch (pcb_rev) {
		case 0:
			return "NanoPi R2";
		default:
			return "FriendlyELEC RK3328 board";
	}
}
