...
static inline int read_deviceinfo_storage(struct hdmi_dev *hdmi_dev)
{
	int ret = 0;
	const disk_partition_t *ptn_deviceinfo;
	char *p_deviceinfo = (char *)(CONFIG_RKHDMI_PARAM_ADDR);
	char *p_baseparamer =  (char *)(CONFIG_RKHDMI_PARAM_ADDR + 0x1000);
	...
	if (gd->fdt_blob) {
		...
		deviceinfo_reserve_on =
			fdtdec_get_int(gd->fdt_blob, node, "rockchip,uboot-deviceinfo-on", 0);
...
/*
 * dw_hdmi_vic2timing: transverse vic mode to video timing
...
static int hdmi_videomode_to_vic(struct hdmi_dev *hdmi_dev, struct video_mode *vmode)
{
	...
	for(i = 0; i < hdmi_dev->mode_len; i++) {
		mode = (const struct video_mode *)&(hdmi_dev->modedb[i].mode);
		if (mode == NULL) {
			printf("%s: NULL mode\n", __func__);
			return -EFAULT;
		}
...
static const struct video_mode vesa_modes[] = {
	/* 0 640x350-85 VESA */
...
static int get_est_timing(unsigned char *block, struct video_mode *mode)
{
	int num = 0;
	unsigned char c;
...
static int get_std_timing(unsigned char *block, struct video_mode *mode,
		int ver, int rev)
{
...
static int get_dst_timing(unsigned char *block,
			  struct video_mode *mode, int ver, int rev)
{
	int j, num = 0;
 ...
 *
 * RETURNS: struct video_mode, @dbsize contains length of database
 *
 * DESCRIPTION:
 * This function builds a mode database using the contents of the EDID
 * data
 */
static struct video_mode *fb_create_modedb(unsigned char *edid, int *dbsize)
{
	struct video_mode *mode, *m;
	unsigned char *block;
	int num = 0, i, first = 1;
	int ver, rev;
	...
	mode = malloc(50 * sizeof(struct video_mode));
	if (mode == NULL)
		return NULL;
	...
	*dbsize = num;
	m = malloc(num * sizeof(struct video_mode));
	if (!m)
		return mode;
	memmove(m, mode, num * sizeof(struct video_mode));
	free(mode);
	return m;
}
 ...
 */
static int hdmi_edid_parse_dtd(unsigned char *block, struct video_mode *mode)
{
	mode->xres = H_ACTIVE;
	mode->yres = V_ACTIVE;
	mode->pixclock = PIXEL_CLOCK;
...
//					pedid->ycbcr420 = 1;
	...
	// Parse DTD
	struct video_mode *vmode = malloc(sizeof(struct video_mode));
	if(vmode == NULL)
		return E_HDMI_EDID_SUCCESS;
	while(ddc_offset < HDMI_EDID_BLOCK_SIZE - 2)	//buf[126] = 0 and buf[127] = checksum
	{
		if(!buf[ddc_offset] && !buf[ddc_offset + 1])
			break;
		memset(vmode, 0, sizeof(struct video_mode));
...
static void hdmi_find_best_edid_mode(struct hdmi_dev *hdmi_dev)
{
	int i = 0, pos = 0;
	struct hdmi_video_timing *modedb;
...
void dw_rk_hdmi_register(struct hdmi_dev *hdmi_dev)
{
	int ret = 0;
	...
	if (ret)
		printf("%s:fail to read baseparameter\n",__func__);
}
